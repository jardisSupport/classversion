...
class ClassVersionTest extends TestCase
{
    ...
    // =========================================================================
    // Optional ClassResolutionCache integration
    // =========================================================================

    public function testCacheCausesReaderToBeCalledOnlyOncePerKey(): void
    {
        $config = new ClassVersionConfig();
        $calls = 0;
        $classFinder = new class ($calls) implements ClassVersionInterface {
        ...
        $classVersion = new ClassVersion($config, $classFinder, cache: $cache);

        $classVersion(VersionClass::class);
        $classVersion(VersionClass::class);
        $classVersion(VersionClass::class);
    ...
    public function testCacheDistinguishesVersions(): void
    {
        $config = new ClassVersionConfig();
        $calls = 0;
        $classFinder = new class ($calls) implements ClassVersionInterface {
        ...
        $classVersion(VersionClass::class, 'v1');
        $classVersion(VersionClass::class, 'v2');
        $classVersion(VersionClass::class, 'v1');
        $classVersion(VersionClass::class, 'v2');

        $this->assertSame(2, $calls, 'cache must separate versions but not re-run same key');
    }

    public function testCacheIsOptionalDefaultBehaviorUnchanged(): void
    {
        $config = new ClassVersionConfig();
        $classFinder = new LoadClassFromSubDirectory();

        // Explicitly no cache — existing constructor signature still works.
        $classVersion = new ClassVersion($config, $classFinder);
